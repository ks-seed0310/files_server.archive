...
#Quiz1 v1.1
#Quiz1 version: Version 1.1.1.0 / v1.1.1 Release
import random as rd
import math as Math
...
def command():
    data["temporary_5"]=input("コマンドを入力")
    if data["temporary_5"]=="/command quiz view":
        print("\n",QuizData[0],"\n")
    if data["temporary_5"]=="/command quiz hint":
        if QuizData[4]:
            print("\nヒント:\n",QuizData[4],"\n")
        else:
            print("\nヒントは登録されていません。\n")
    ...
    for count_____ in range(1,data["quiz_count"]+1):
        ...
        while True:
            try:
                ...
                if data["temporary_2"]=="/command":
                    command()
                    continue
                
                if (("".join(data["temporary_2"].split()))==""):
                    continue
                if QuizData[3]:
